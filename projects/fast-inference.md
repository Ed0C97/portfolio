...
- **GPU-level performance engineering**: hand-written Triton kernels (fused RMSNorm/LayerNorm using single-pass Welford statistics, tiled attention with online softmax, fused SwiGLU activations), written to cut the HBM round-trips that dominate the small-batch, memory-bound regime typical of RAG. They are an optimization and ablation layer rather than part of the serving path: they compile for CUDA only, nothing under models/ or server/ imports them, and no committed measurement yet attributes a latency change to them.
...
- **Distributed serving components**: a latency- and queue-depth-weighted router, a three-state circuit breaker with periodic health checks, and a subprocess worker manager, each unit-tested. Implemented but standalone: no gateway wires them into the request path yet, so a running server today is a single process.
