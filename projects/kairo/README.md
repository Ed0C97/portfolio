...
A few self-contained excerpts from Kairo, a Kotlin Multiplatform + native-SwiftUI iOS/Android app. They show platform-craft (bridging native cryptography from common code) and UI-craft (a small, consistent SwiftUI design system) without exposing any product logic.

**Context:** see the project page → [../repo-readmes/kairo.md](../repo-readmes/kairo.md)
...
- **`SecureStorageIos.kt`** — the iOS `actual`: Keychain access via cinterop. Builds Core Foundation query dictionaries with `CFDictionaryCreate` over the `Security.framework` `CFString` constants, handles `NSData` ↔ `ByteArray` marshalling, `memScoped` result pointers, delete-before-add for overwrite semantics, and `AfterFirstUnlockThisDeviceOnly` accessibility.
...
- All repositories, use cases, and DI wiring that consume `SecureStorage` — i.e. the actual app/domain logic.
