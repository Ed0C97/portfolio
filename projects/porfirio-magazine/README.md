...
- **`carousel_generator.py`**: A shipped, pragmatic Gemini AI pipeline that turns an article into an Instagram marketing carousel end-to-end: AI copy generation (retry with falling temperature, JSON-recovery parsing, validate and auto-fix to fit the layout, graceful fallback), then headless-browser HTML render, then screenshot, then Cloudinary upload, plus a sibling SHA-256 content-hash translation cache that only re-calls the model when the source actually changes.
