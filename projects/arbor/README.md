# ARBOR: AI Search and Recommendation Engine
...
reader will love. Under the hood it is a retrieval and ranking system: it has
...
candidates through stages of increasing cost: dense cosine, then BM25 (fused via
RRF), then cross-encoder rerank, then an LLM-as-judge pass. Cheap stages run first
...
A request flows roughly: `hybrid_search` retrieves a broad candidate set, then
`reranking_pipeline` narrows and reorders it for relevance, then `ranking`
