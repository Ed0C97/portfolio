...
- **Symbolic verification of financial constraints**: figures and relationships extracted from a document are compiled to a formula AST, evaluated with SymPy, and checked for satisfiability by an ensemble of SMT solvers (Z3, CVC5, dReal), so an inconsistency is proven rather than guessed at by a second model pass. Unsatisfiable constraint sets come back as located boundary violations. When backends disagree the goal is marked divergent and surfaced for human review rather than settled by majority vote, on the principle that solver disagreement is where real correctness problems hide. Separate property and proof backends cover checks that do not reduce to SMT.
...
Beta and actively developed, on calendar versioning. Built as a Python monorepo (two FastAPI applications plus a web UI, an Electron desktop app, and a Python SDK) backed by PostgreSQL, Neo4j, and Pinecone, and part of a coordinated multi-repository ecosystem of independently-versioned toolkit packages. The test suite runs 6,569 test functions across 701 files (unit, integration, property, consistency, determinism), behind CI that gates evaluation and grounding on every change, runs formal-verification and TLA+ specification checks on push, and sweeps with scheduled mutation-testing and red-team runs. Sole architect and developer.
